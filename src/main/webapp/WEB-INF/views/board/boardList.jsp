...
<div class="container">
	<div class="row">
		...
		<div>
			<ul class="pagination">
			<c:if test="${pager.curBlock>1}"><li><a href="./${board}List?curPage=${pager.startNum-1}">이전</a></li>
			</c:if>
			<c:forEach begin="${pager.startNum}" end="${pager.lastNum}" var="i">
				<li><a href="./${board}List?curPage=${i}">${i}</a></li>
			</c:forEach>
			<c:if test="${pager.totalBlock>pager.curBlock}"><li><a href="./${board}List?curPage=${pager.lastNum+1}">다음</a></li>
			</c:if>
		</div>
			</ul>
